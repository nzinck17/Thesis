...
```{r res, echo=FALSE, out.width = "6.5in", fig.align="center", fig.cap="Water Quality Parameters Sampled within Tributaries and Reservoirs"}
...
\begin{table}
\centering
\caption{R Packages Utilized in Applications}
\label{table-paramvalues}
...
```{r WIT Session user, echo=FALSE, out.width = "6.5in", fig.cap="Flow of WIT User Session"}
...
```{r WIT Session start, echo=FALSE, out.width = "6.5in", fig.cap="Flow of events before launching WIT"}
...
```{r WIT Session proc suc, echo=FALSE, out.width = "6.5in", fig.cap="Flow of events during WIT succesfully processing data"}
knitr::include_graphics("images/WIT/WIT_session_proc_suc.pdf")
```

```{r WIT Session proc fail, echo=FALSE, out.width = "6.5in", fig.cap="Flow of events during WIT unsuccesfully processing data"}
...
```{r WIT Session import, echo=FALSE, out.width = "6.5in", fig.cap="Flow of events during WIT importing data"}
...
```{r WIT Session exit, echo=FALSE, out.width = "6.5in", fig.cap="Flow of events after a user exits WIT"}
...
It is very advantageous for all DCR staff to have fast and easy access to water system data. In many organizations this is not the reality due to data not being stored in a common location known by all staff members. Lack of experience with certain technologies can also inhibit a person from being able to timely access data. WAVE helps make it possible for all DCR staff to get easy access to the data they want and need.

WAVE queries the data based on the user-selected input widgets. The input widgets are divided into two sections, selections and filters. In general, "selections" are inclusive where a user selects choices to add to the selected data observations (e.g. Site, Parameters, Date). If any of the selection widgets are empty, no data will be selected. In general, filters refine the data, removing unwanted data from the previously selected data (e.g. Flags ). A user will not have to select or update any advanced filters in order for data to be selected.

The user is prompted to first select one or more Site Locations with a map that indicates which sites are selected. A site map will show the available sites and indicate which sites on the map have been selected. The user is then prompted to select the parameters and date range or years for the selected sites. Additional filters can also be applied to the data which include selection of seasons, months, years, flags, storm event, and other eventually meteorological and hydrological conditions. Queried data can be exported to a csv file with the click of a button.

There is an interactive table that the user has. The filter module also has the option to select all data, which if the user selects this preference, none of the filters will be used and the whole dataframe will be selected. 

## Data Visualization

WAVE allows a user to visualize water quality and quantity data to better interprete findings and trends. As humans are visual creatures, it is often hard for us to draw detailed conclusions about data by just looking at it in a table. The user is offered multiple types of visualizations for different types of data. These visualizations include temporal, correlation, and distribution plots. Most of the plots created in WAVE have interactive features on the plot itself in which the user can hover over a data point to display popup information and also toggle the plot in various ways like zooming in and out.

The user is given many options to customize how each plot displays the data. The user is given axes preferences where applicable, including an option for linear or logarithmic scales and whether to force an axis to begin at zero. In general, the user can adjust the size of points, lines, and other shapes that represent data or data trends. The user is given options to adjust the point color or color scale of the data in plots. The user can choose form many display themes that are offered in ggplot. The plot automatically creates logical axis labels and a descriptive plot title, which the user can override with custom text or choose to have no labels or title. Where applicable, the user can add a horizontal lines, vertical lines, or floating text to a plot anywhere they specify.  

The user can save a plot for use outside of WAVE which they can share among peers or add to a report. The file can be saved in multiple formats (e.g. pdf, jpg, or pdf) with a user specified plot width and height. The plots in this chapter (Figures 6.1 to 6.5) are examples of plots that were created in WAVE and saved through the saving feature of WAVE.
...
```{r WAVE plot time, echo=FALSE, out.width = "6.5in", fig.cap="Time Series Plot generated with WAVE showing the Discharge and SPecific Conductance for three major tributaries of the Wachusett Reservoir. The data represents the discharge and flow form the Stillwater River, Quinapoxet River, and Gates Brook for the years 2015 and 2016."}
...
```{r WAVE plot corr, echo=FALSE, out.width = "6.5in", fig.cap="Time Series Plot generated with WAVE showing the Discharge and SPecific Conductance for three major tributaries of the Wachusett Reservoir. The data represents the discharge and flow form the Stillwater River, Quinapoxet River, and Gates Brook for the years 2015 and 2016."}
knitr::include_graphics("F:/Nick Zinck/Shiny Apps/GitHub/Thesis/Thesis/images/WAVE features/plots/Corr.png")
```

### Distribution

The distribution of Results of a particular water quality parameter can be visualized in the application. Based on the user selected Locations, Parameter, and Date Range, the user can create a histogram, Density Plot, or box plots. The user has many grouping options in which they can choose from. The user can choose to "color" or facet by "None", "Site", "Year", "Month", or "Season" with the click of a button.

```{r WAVE plot dist 1, echo=FALSE, out.width = "6.5in", fig.cap="Histogram generated with WAVE showing the distribution of specific Conductance for three major tributaries of the Wachusett Reservoir (Stillwater River, Quinapoxet River, and Gates Brook) for the years 2015 and 2016. Colors are used to represent the  grouping and facets are used to represent the  grouping "}
knitr::include_graphics("F:/Nick Zinck/Shiny Apps/GitHub/Thesis/Thesis/images/WAVE features/plots/Dist-1.png")
```

```{r WAVE plot dist 2, echo=FALSE, out.width = "6.5in", fig.cap="Histogram generated with WAVE showing the distribution of specific Conductance for three major tributaries of the Wachusett Reservoir (Stillwater River, Quinapoxet River, and Gates Brook) for the years 2015 and 2016. Colors are used to represent the  grouping and facets are used to represent the  grouping "}
knitr::include_graphics("F:/Nick Zinck/Shiny Apps/GitHub/Thesis/Thesis/images/WAVE features/plots/Dist-2.png")
```

```{r WAVE plot dist 3, echo=FALSE, out.width = "6.5in", fig.cap="Histogram generated with WAVE showing the distribution of specific Conductance for three major tributaries of the Wachusett Reservoir (Stillwater River, Quinapoxet River, and Gates Brook) for the years 2015 and 2016. Colors are used to represent the  grouping and facets are used to represent the  grouping "}
knitr::include_graphics("F:/Nick Zinck/Shiny Apps/GitHub/Thesis/Thesis/images/WAVE features/plots/Dist-3.png")
```

```{r WAVE plot dist 4, echo=FALSE, out.width = "6.5in", fig.cap="Histogram generated with WAVE showing the distribution of specific Conductance for three major tributaries of the Wachusett Reservoir (Stillwater River, Quinapoxet River, and Gates Brook) for the years 2015 and 2016. Colors are used to represent the  grouping and facets are used to represent the  grouping "}
...
A Heatmap is useful to visualize profile reservoir data at a specific location throughout the year. A heatmap is an interpolated color matrix plot which will interpolate the values between matirc points to create a smooth color map. WAVE uses a heatmap to represent depth on the y-axis, time on the x-axis, and the values of a particular parameter indicated by a color range. The user can also choose to make this a 3 dimensional heatmap as shown in figure .
...
Profile Line Plots are used to show profile data similar to the heat plot. The Line plot better maintains the integrity of the data, since it is less interpolated, yet may be more visually dificult to interprete. There are benefits of both Line plots and heatmaps. Future work may include haveing the ability to show data as points rather than just as connected by lines.
...
```{r WAVE plot heatmap 3d, echo=FALSE, out.width = "6.5in", fig.cap="Three-dimensional heatmap generated with WAVE showing the specific conductance (uS/cm) in the Wachusett Reservoir at the Basin North Station for the year 2015. Specific conductance profile data is collected approximately every week and was interpoloted to create this heatmap."}
...
There are many plots available tohelp visualize phytoplankton data with minimal user effort. IN most cases the user will just have to select the year of the data that they are interested in and many plots will then be generated.
...
```{r WAVE plot phyto ura taxa, echo=FALSE, out.width = "6.5in", fig.cap="Phytoplankton Overview Plot generated with WAVE showing phytoplankton density and secchi transparency depth at two Wachusett Reservoir Stations, Basin North and Cosgrove Intake, for 2017."}
knitr::include_graphics("F:/Nick Zinck/Shiny Apps/GitHub/Thesis/Thesis/images/WAVE features/plots/Phyto-ura-taxa.png")
```

```{r WAVE plot phyto ura hist, echo=FALSE, out.width = "6.5in", fig.cap="Heatmap generated with WAVE showing the temperature (Celsius) in the Wachusett Reservoir at the Basin North Station for the year 2015. Temperature profile data is collected about every week and was interpoloted to create this heatmap."}
...
Data science technologies are advancing rapidly which means more efficient methods of dealing with data are available. Much of this technology is available for free and is open source. Companies and industries adopt these technologies at different rates presumably due largely to enconomic incentives. Organizations that utilize the latest data science technologies will likely be more productive and efficient at dealing with their data than those that do not. It is important to adopt new technologies and develop a system to encourage future growth by adopting even newer technologies when timing is ideal. Not all new technology is better, but as more options become available, there will likely be tools that are developed which may fit ones needs better than the previous one that they are using.

It seems like WIT and WAVE will be integral at the DCR for data entry, query, visualization, and analysis in the foreseable future. Debatably more important that the current application today, is the data organization and thought anout data more.  that this work has . Much unseen effort was put into building (and rebuilding) the application into a well structured organized application built in a way to allow for efficient updates.  The tabular layouts allow for an independent module to be attached to the existing applicaitn.  One can work on an independent module, not attached to the larger applicaiton and the. Shiny is beneficial becuase it is a relatively easy to learn and has a finite amount of learnign associated with it. Python is another Language that could be considered if one was to undergo another application, though it seems like R and Shiny were the best choice for this project. Hopefully in the future, several people are knowlegeable on how the application works and is . Collaboration is very easy with an applcition like this and should be taken advantage of.

This project has really set the foundation for the WIT and WAVE application, yet there is growth to come for these applications. There is already a long list of potential new features to add to WAVE. Some of these features are currently in the development includeing Hydrology and Meteorology tabs which involve stage, rating curves, loading, and precipitation. Other features that may come in the foreseaable future include report generation, more advanced statistical analysis (e.g Mann-Kendall statistic), and and the inclusion of forestry data and MWRA data. 
A system 


This also includes having a system in place for sufficient growth in the future. Data needs to be in the apprporaite accessible format to be utilized with these programs. A proper system should be set in place to allow for. less time on day to day and more time on analyzing and visualizing data. This new data can also lead to smarter data gathering methods and further improved efficieny which thus frees up more time for studies. The 


It is important that a system is put in place to allow for data science to occur efficiently and to allow for the grouwth in the future. The development of a dynamic application that can adapt to changing technologies is a great feature to have. Moving away from daily spreadsheet storage and computations and through automation is a huge step that many organizations should take in making their activities more productive.
...
The insights that WAVE will bring will hopefully modify the direction of WIT and WAVE. ew insights could lead to further invesitgation and more specific visualization and analysis techniques which will be incorporated into WAVE. New insights could alter sampling routine or procedures which will have an effect on how WIT operates. 

Will help with emerging Water qulaity concerns.
