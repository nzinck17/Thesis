...
The application is a relatively simple application network. Figure 3.1 depicts a simplified version of the system components. The user opens the application and the server runs in the background. The server is Shiny that both executes R code for data computations and creates the user interface through rendering javascript and HTML. The user (client) sends information to the server through interactions with the user interface and the server will update the user interface accordingly. The server also will reach can fetch and retirieve data from the database (or intermediary data storage).

```{r dev system 1, echo=FALSE, out.width = "4in", fig.cap="Needs to be changed!! (incorrect data!)"}
knitr::include_graphics("images/development/system1.pdf")
```
